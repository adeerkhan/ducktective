  ...
  return (
    <SiteShell>
      ...
      <p className="mt-4 max-w-2xl text-base leading-relaxed text-muted">
        ...
        cross-examination. Ducktective is the second job, packaged as a drop-in skill for the agent
        you already run.
      </p>
      ...
      <article className="mt-12 max-w-3xl space-y-12 text-[15px] leading-relaxed">
        <section>
          ...
          <p className="mt-3 text-muted">
            ...
            <em className="text-fg not-italic">Silent semantic failure</em> (arXiv:2603.25764):
            across 1,750 trajectories on 50 SWE-bench Verified tasks, GPT-5 submitted a patch on
            100% of runs and resolved 44%. Sixty-eight percent of GPT-5’s failing runs were the same
            wrong interpretation, repeated. Lightweight pre-edit prompts did not close the gap.
            Given an already-fixed bug, most models still edited the correct code. Completion
            metrics reward that instinct. Ducktective forbids it.
          </p>
          <p className="mt-3 text-muted">
          ...
          <p className="mt-3 text-muted">
            ...
            3–5 → falsify with a runnable check → emit a case file → remember the case in this repo.
            Localization, graphs, and the patch itself stay with systems that already do them
            better.
          </p>
        </section>
        ...
        <section>
          ...
          <ol className="mt-4 space-y-3 text-muted">
            <li>
              <span className="text-fg">Reproduce-or-stop.</span> The failing command is the only
              admission ticket. If it is green, the case is stamped DOES NOT REPRODUCE and the skill
              exits. This is the already-fixed probe from the silent-failure paper, turned into a
              hard gate.
            </li>
            <li>
              <span className="text-fg">Fail-only coverage, not a graph.</span> Stack frames plus
              lines hit by the failing run and not by passing runs — classic cheap SBFL. Cap at
              five. Tree-sitter is optional and disposable. LocAgent’s heterogeneous graph is a
              research artifact (627 stars, ACL 2025). Rebuilding an incomplete one is busywork.
            </li>
            <li>
              <span className="text-fg">Hypothesis then check, never the reverse.</span> One
              sentence: “this function should return X under Y, the failing run shows Z.” Then the
              smallest assertion that would disprove it. Execute. If the oracle holds, the candidate
              is falsified. If it fails in the predicted way, the cause is confirmed. Chat
              confirmation is discarded.
            </li>
            <li>
              <span className="text-fg">Differential oracle when a green path exists.</span> A check
              that also fails on passing tests is a bad check. Ducktective prefers oracles that
              distinguish the failing run from a known-good one. That is the whole novelty: not a
              smarter model, a check that can lose.
            </li>
            <li>
              <span className="text-fg">The case file is the product.</span> Structured, stamped,
              append-only. Humans merge from it. Suggested patch is labeled secondary so the skill
              cannot launder a guess as a fix.
            </li>
          </ol>
        </section>
        ...
        <section>
          ...
          <p className="mt-3 text-muted">
            ...
            Agent Skills standard (`SKILL.md`) loads in Claude Code, Cursor, Copilot, Codex, Gemini
            CLI, and thirty others. One folder. One command.{" "}
            <span className="text-fg">ducktective investigate</span>. It fires on the sentence
            people already type: “this test is failing, find out why.”
          </p>
          <p className="mt-3 text-muted">
            That is why it can out-star LocAgent and sit with Agentless / AutoCodeRover without
            beating OpenHands — it is not trying to. A skill that stops non-reproducing cases,
            leaves a docket a reviewer will actually read, and costs almost no tokens versus “just
            fix it” has a daily trigger. Daily trigger plus one-command install plus a named failure
            mode in the 2026 literature is the star recipe. Costume detective graphs are not.
          </p>
        </section>
        ...
        <section>
          ...
          <p className="mt-4 text-sm text-muted">
            Out of scope for two weeks: custom graphs, multi-agent, monorepos, production telemetry,
            metamorphic oracles. Those wait until the spine is measured.
          </p>
        </section>

        <section>
          <h2 className="font-display text-2xl tracking-tight">
            What this workbench already proves
          </h2>
          <p className="mt-3 text-muted">
            Five JavaScript faults execute here. Off-by-one, leaked mutable cache, inverted boolean,
            UTC-as-local, and a stale ticket whose test is already green. Each investigation
            collects fail/pass hits, runs the falsifying check, and stamps a case file onto the rap
